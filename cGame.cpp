...
bool cGame::Init(int lvl)
{
	...
	glMatrixMode(GL_PROJECTION); //switches the matrix mode between projection and model view
	glLoadIdentity(); //performs an identity upon the current matrix.
	...
	//Sound initialization NOPE MUTED
...
{
	if(key >='A' && key <= 'Z') key += 32; //inputting uppercase chars converts to lowercase

	//	when you release a side arrow key when scrolling diagonally, player frames can change
	...
	//	the moment the skill key is pressed the load is lost
...
//only triggers on mouse input which is a problem.
void cGame::ReadMouse(int button, int state, int x, int y)
{
	//when the mouse is clicked, character attacks. 
	if (button == 0 && state == 0) p1_attacks = true;
}

//only triggers on mouse input which is a problem.
void cGame::ReadMousePassive(int x, int y)
{
	//grab coordinates of mouse location.
	mouseX = x;
	mouseY = y;
}
...
bool cGame::Process()
{
	...
	int h1, h2, w1, w2, calc1, calc2;


	/*judges angle between mouse coordinate and player coordinate to determine trajectory of projectiles
	and sprite art.*/
	h2 = Player2.GetY();
	w2 = Player2.GetX();
	...
	w1 = Player1.GetX();
	calc1 = mouseX - w1;
	calc2 = abs(432-mouseY) - h1; //change 432 to actual game height in the future.
	radian = atan2(calc1, calc2);

	//set state of where player is looking.
	if (radian > -0.75 && radian < 0.75 ) {
		Player1.SetState(STATE_LOOKUP);
	}
	else if (radian > .75 && radian < 2.25) {
		Player1.SetState(STATE_LOOKRIGHT);
	}
	else if (radian > 2.25 || radian < -2.75) {
		Player1.SetState(STATE_LOOKDOWN);
	}
	else 
	{
	...
	if(!Player1.IsAttacking() && !Player1.IsDamaged())
	{
		...
		if(doing_skill) Player1.SetState(Player1.GetLogicState()); //when a skill is active, the state stays in a fixed direction so instead of the state attribute, I use the logic_state of Player1 to be able to calculate the movement and the slide
		...
		Player1.SetLogicState(Player1.GetState());  //After the possible movements I update the logic_state of Player1 in case it is necessary to use it in the next iteration of the gameloop
		if(!doing_skill) input_state = Player1.GetState(); //in this case input_state will never be a state of skill
