...
{	
	...
	switch(GetState())
	{
		...
		case STATE_WALKUP:		xo = tex_offset_x*1;   
								yo = tex_offset_y + tex_offset_y*1 + (GetFrame()*tex_offset_y);
								if(run) NextFrame(4);
								break;
		case STATE_WALKRIGHT:	xo = tex_offset_x*2;   
								yo = tex_offset_y + tex_offset_y*1 + (GetFrame()*tex_offset_y);
								if(run) NextFrame(4);
								break;
		case STATE_WALKDOWN:	xo = tex_offset_x*0;   
								yo = tex_offset_y + tex_offset_y*1 + (GetFrame()*tex_offset_y);
								if(run) NextFrame(4);
								break;
		case STATE_WALKLEFT:	xo = tex_offset_x*2;   
								yo = tex_offset_y + tex_offset_y*1 + (GetFrame()*tex_offset_y);
								if(run) NextFrame(4);
								reverse = true;
								break;
...
void cPlayer1::SetShieldState(int input_state)
{
	...
	//if the input_state was not a skill we transform it into a skill
